...
- **Build:** `build-release/` is current. 650 C++ tests pass (`./lob_tests`). 2221 Python tests pass (1308 core + 913 barrier). **2871 total.** (15 C++ + 59 Python skipped — need `.dbn.zst` fixture.)
...
- **Barrier cache FRESH:** `cache/barrier/` has 248 `.npz` files with 220-dim features + per-bar features (N_FEATURES=22). Re-precomputed via C++ backend. 461K total bars, 456K usable. 221 MB cache. Now includes `bar_features` key (n_trimmed, 22) for sequence models.
...
- **Per-bar feature cache DONE (PR #47):** C++ `barrier_precompute()` now outputs `bar_features` (n_trimmed, 22) float32 and `n_trimmed` int alongside existing lookback-assembled features. `load_session_features()` added to `first_passage_analysis.py`. 14 C++ + 25 Python tests.
- **Sequence model infrastructure DONE (PR #48):** `sequence_models.py` — `BarEmbedding`, `LinearBarEmbedding`, `BarrierLSTM`, `BarrierTransformer`, `collate_sessions`, `train_sequence_model`, `predict_sessions`, `evaluate_sequence_model`. Swappable embedding interface, causal Transformer, masked BCEWithLogits loss, AdamW+cosine, early stopping. 49 tests.
- **exp-007 REFUTED:** LSTM and Transformer on full-session bar sequences both fail to beat constant Brier. All 8 (model, label, split) BSS values negative. Best: transformer/short BSS = -0.0004 (p=0.791). LSTM is *worse* than flat LR (BSS -0.0173 vs -0.0003). Transformer collapses to near-constant predictions (p̂_std=0.014). Six model families tested; all fail. PR #49.
- **exp-008 REFUTED:** Bar-size sweep (B ∈ {200,500,1000,2000} × R ∈ {1x,2x,3x}) — no (B,R) config produces BSS ≥ 0.005. Best BSS = 0.0023 (B=200/R=4/long, p=0.0) but 2.3× below threshold. 20/24 cells negative. BSS degrades monotonically with bar size. Phase 2b complete.
- **Next task: Conditional signal detection or pivot.** Three axes exhausted (architecture, scale, data quantity). Remaining options: (1) conditional signal (regime-filtered LR), (2) alternative targets (1:1 barriers), (3) feature pivot (external context), (4) accept null and pivot to execution-timing RL. See `results/exp-008-bar-size-sweep/analysis.md`.
...
- **Architecture exploration CLOSED:** Six model families tested (LR, GBT, MLP, RF, LSTM, Transformer) — none beat constant Brier. Architecture cannot amplify signal that isn't there.
...
- **Experiments completed:** 13 total (2 confirmed, 11 refuted). See `RESEARCH_LOG.md`.
- **PRs this session:** #47 (per-bar features), #48 (sequence models), #49 (exp-007 sequence model signal detection).
