...
- **Next task: Implement the Asymmetric First-Passage Trading plan** — `experiments/Asymmetric First-Passage Trading.md`. This is a 5-phase research plan that reframes the problem using Brier scores, proper calibration, and information-theoretic bounds. Phase 0 (data pipeline) mostly exists. Phase 1 (null calibration: ȳ ≈ 1/3?) and Phase 2 (signal detection: logistic regression + GBT baselines beating constant Brier score) are the immediate work. **Key shift: binary labels Y_long, Y_short ∈ {0,1} as independent predictions, NOT the three-class {long, short, flat} used in T6/exp-004.**
