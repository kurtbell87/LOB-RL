...
SRC_DIR="${SRC_DIR:-python/lob_rl}"                   # Model / training code
DATA_DIR="${DATA_DIR:-cache/mes}"                     # Datasets (precomputed .npz cache)
...
# Commands -- LOB-RL project-specific
TRAIN_CMD="${TRAIN_CMD:-cd build-release && PYTHONPATH=.:../python uv run python ../scripts/train.py --cache-dir ../cache/mes/ --bar-size 1000 --execution-cost --policy-arch 256,256 --activation relu --ent-coef 0.05 --learning-rate 0.001 --shuffle-split --seed 42}"
EVAL_CMD="${EVAL_CMD:-cd build-release && PYTHONPATH=.:../python uv run python ../scripts/train.py --eval-only}"
TEST_CMD="${TEST_CMD:-PYTHONPATH=build-release:python uv run pytest python/tests/}"
