...
| P0 | Does conditional signal detection (regime-filtered) produce BSS > 0? | Not started | — | — | If BSS > 0 in specific regimes: signal is conditional, not absent. Motivates regime-aware RL. If no: current features are genuinely uninformative. |
...
| Does signal exist at a different bar-size/barrier-width scale? | REFUTED | No — 4 bar sizes × 3 risk calibrations (24 cells) swept. Best BSS = 0.0023 (B=200/R=4/long), 2.3× below the 0.005 meaningful threshold. Statistically significant only due to 1.15M samples. 20/24 cells have negative BSS. BSS degrades monotonically with bar size. The 22-feature obs space does not contain exploitable signal at any tested scale. | exp-008-bar-size-sweep in RESEARCH_LOG.md |
